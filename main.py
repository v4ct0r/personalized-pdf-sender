...
try:
    ...
    custom_exit("transactions directory not found")
except Exception as e:
    custom_exit(f"Error: {e}")
...
for client in clients:
    if clients[client]["valid_email"]:
        ...
        if(send_email(clients[client])):
            #print(f'Email sent to {clients[client]["email"]} successfully')
            move_pdf_files(clients[client], 'processed_transactions')
        # print all pdf paths
        #print(f'PDFs: {clients[client]["pdf_paths"]}')
